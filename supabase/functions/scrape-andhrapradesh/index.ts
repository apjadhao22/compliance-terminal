...
const SEARCH_QUERY = "Andhra Pradesh government labour notification circular 2025";
const SOURCE_NAME = "Andhra Pradesh Government";
...
serve(async (req) => {
  ...
  try {
    ...
    for (const item of toProcess) {
      ...
      await runPipeline({ title: item.title.slice(0, 200), url: item.url, rawText, sourceName: SOURCE_NAME, state: STATE, documentType: "notification" });
      processed++;
      await sleep(3500);
    }
